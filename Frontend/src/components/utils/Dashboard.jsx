  ...
  useEffect(() => {
    ...
    const fetchProfilePic = async () => {
      try {
        ...
        setProfilePicture(response.data.picUrl); // Use the URL from backend
      } catch (error) {
        console.log('Error while fetching pic :', error.response?.data?.message || error.message);
      }
    };
    fetchProfilePic();
  }, []); // Removed profilePicture from dependency array to prevent re-fetching

  const handleFileChange = async (e) => {
    const file = e.target.files[0];

    // --- FIX: Add a guard clause ---
    // If no file is selected (e.g., user hits 'cancel'), do nothing.
    if (!file) {
    ...
    try {
      ...
      // --- UPDATE: Update the profile picture in state immediately ---
          ...
          <div className="relative w-32 h-32 rounded-full overflow-hidden bg-gray-200 border-4 border-white shadow-lg group">
            {profilePicture ? (
              <img
                ...
                // Add an error fallback just in case
                onError={(e) => { e.target.src = 'https://i.ibb.co/3ywTZFTd/download.png'; }}
              />
            ) : (
            ...
            <div className="absolute inset-0 bg-black bg-opacity-50 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer space-y-2">
              {/* Don't show view button if it's the default pic */}
              ...
              <button
                ...
                title="Change Image"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.86-1.638A2 2 0 0110.45 3h3.1a2 2 0 011.664.89l.86 1.638A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2v-9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              </button>
            </div>
          </div>
          <input
            ...
            <li className="bg-gray-50 p-4 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center">
              <div className="flex-1">
                <p className="font-medium">
                  Donation to "Environmental Protection"
                </p>
